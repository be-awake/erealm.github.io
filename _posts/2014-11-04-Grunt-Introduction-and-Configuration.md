...
##二、安装Ggrunt插件
&emsp;&emsp;通过**npm install <module> --save-dev**命令向已经存在的package.json文件中添加Grunt和grunt插件。此命令不光安装了 module ，还会自动将其添加到devDependencies配置段中。所以网站中，运用下面这条命令将安装Grunt最新版本到项目目录中，并将其添加到 devDependencies内：    
